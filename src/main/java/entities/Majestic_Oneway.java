...
import TestScript.Arivo.ArivoOneway;
import TestScript.Majestic.MajesticOneway;
import utilities.Logger;
import utilities.QaRobot;

public class Majestic_Oneway extends MajesticOneway {

	@SuppressWarnings("deprecation")
	public static void flightoneway(String ddate, String Triptype, String adult, String child, String infant,
			String fcurrency, String fclass, String airlines, String fbooking, String emailid, String title,
		...
		String Fclass = "xpath://select[@id='ddlClass']/option[text()='" + fclass + "']";
		String result = "";
		...
		QaRobot.ClickOnElement("gobdgo_pax", "Clicked on Pax");

		// select adult

		QaRobot.selectTextFromDropdown("Arivo_adult", adult, "Selected Adult");

		// select child

		QaRobot.selectTextFromDropdown("Arivo_child", child, "Selected Child");

		// select infant

		QaRobot.selectTextFromDropdown("Arivo_infant", infant, "Selected Infant");

		Thread.sleep(200);
		
		//clicked on interface
		
		driver.findElement(By.xpath("//span[@id='traveller_span']")).click();
		
		QaRobot.ClickOnElement("Majestic_moreoption", "Clicked on more option");
		
		//select currency
		
		QaRobot.selectTextFromDropdown("Majestic_currency", fcurrency, "Selected Currency");

		// select class

		QaRobot.selectTextFromDropdown("Majestic_class", fclass, "Selected Class");
		...
		QaRobot.ClickOnElement("Majestic_search", "Clicked On Serach");
		...
		List<WebElement> ResultNotFound = driver.findElements(By.xpath(
		...
		if (ResultNotFound.size() > 0) {
			...
			try {
				...
				QaRobot.ClickOnElement("Majestic_home", "clicked on Home Page");
			...
			result = driver.findElement(By.xpath("(//span[@class='f_p_n_f_price ng-binding'])[" + fbooking + "]"))
					.getText();
			...
			// explicitwaitvisible(200,
			// By.xpath("//input[@id='ctl00_contentMain_btnSendEnquiry']"));
			...
			// DiscountCoupon("discout_textbox",
			// "discount_msg","discount_amount", couponno,discountvalue);
		...
		// List<File>
		// ListOfLogFile=getLogText1("\\\\release\\QuadXchange\\COM\\Logs\\BookingEngine\\DT\\AirSearch\\SABRE",uid[1]);
